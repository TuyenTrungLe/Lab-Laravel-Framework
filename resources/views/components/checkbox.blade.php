<div class="flex flex-row">
    <label>
        <input {{ $attributes->merge([
            ...
            ]) }} type="checkbox" @if (isset($model)) wire:model="{{ $model }}" @endif @if (isset($name))
            name="{{ $name }}" @endif @if (isset($readonly)) disabled @endif @if (isset($selected) && $selected===true)
            checked @endif @if (isset($value)) value="{{ $value }}" @endif />
    </label>
